...
export function ClayScene({ module, className = '' }: { module: 'recorder' | 'memory' | 'jury'; className?: string }) {
  ...
  return <div role="img" aria-label={label} className={`aspect-[4/3] overflow-hidden bg-[#f7f0e4] ${className}`}><Image src={source} alt="" aria-hidden="true" width={1024} height={768} className="h-full w-full object-cover" /></div>;
}
